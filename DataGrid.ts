...
export function RenderDeviceRows() {
    ...
    for (var i = 0; i < currentPageDevices.length; i++) {
        ...
        if (DeviceUniQID.toLowerCase() == device.ID.toLowerCase()) {
            ...
            recordRow.innerHTML = `
                    ...
                    <td style='display:none;'>${Math.round(device.CpuUtilization * 100)}%</td>
                    <td style='display:none;'>${Math.round(device.UsedStorage / device.TotalStorage * 100)}%</td>
                    <td style='display:none;'>${EncodeForHTML(device.TotalStorage.toLocaleString())}</td>
                    <td style='display:none;'>${Math.round(device.UsedMemory / device.TotalMemory * 100)}%</td>
                    <td style='display:none;'>${EncodeForHTML(device.TotalMemory.toLocaleString())}</td>
                    <td style="white-space: nowrap">
                        <div class="dropdown">
                          ...
                          <div class="dropdown-menu" aria-labelledby="device-actions-dropdown-${device.ID}">
                            ...
                            <a class="dropdown-item device-viewonly-button" href="#" style='display:none;'>
                                <i class="fas fa-eye" title="View Only"></i>
                                View Only
                            </a>
                            <a class="dropdown-item device-edit-button" href="#" style='display:none;'>
                                <i class="fas fa-edit" title="Edit"></i>
                                Edit
                            </a>
                            <a class="dropdown-item device-uninstall-button" href="#" style='display:none;'>
...
export function RefreshGrid() {    
    ...
    //xhr.open("get", location.origin + `/api/Devices/hjhgjghjgjgj`);
