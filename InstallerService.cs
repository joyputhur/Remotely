...
using Remotely.Agent.Installer.Win.WMI;
...
namespace Remotely.Agent.Installer.Win.Services
{
    public class InstallerService
    {
        ...
        {
            Logger.Write("start DownloadRemotelyAgent....");
        ...
        private ConnectionInfo GetConnectionInfo(string organizationId, string serverUrl, string deviceUuid)
        {
            ...
            else
            {
                ...
                {
                    DeviceID = Guid.NewGuid().ToString() // set machine UUID as device ID
                };
            }

            string systemUUID = string.Empty;
            WmiClassCollection classCollection = Wmi.Query("Win32_ComputerSystemProduct", new string[] { "Name", "UUID" });
            if (classCollection != null)
            {
                foreach (var item in classCollection)
                {
                    systemUUID = item["UUID"].Value?.ToString();
                }

                deviceUuid = systemUUID;
            }
